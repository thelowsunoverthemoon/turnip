...
CALL :TURNIP_WINDOW #FFFFFF 900 550 100 100 10

SET "char[alina]=Sprites/char.png Sprites/anger.png Sprites/surprise.png Sprites/shy.png Alina"
SET story="Hmph%TURNIP_EXCL% Are you really thinking I'd go on a date with you%TURNIP_QUESTION% You're crazy%TURNIP_EXCL% Just who do you think you are%TURNIP_QUESTION%%TURNIP_EXCL%" alina[surprise] "I-I mean I'm not trying to be mean, just realistic%TURNIP_EXCL% My parents would never approve." alina[shy] "And it's not l-like you're a bad guy or anything...it's just that I think we're better off as friends." "..." alina[anger] "You know what, fine%TURNIP_EXCL% I-I'll give you a chance, alright%TURNIP_QUESTION% Consider yourself lucky%TURNIP_EXCL%"

FOR /F "tokens=1-5" %%A in ("%char[alina]%") DO (
    CALL :TURNIP_IMG %%E %%A
    
    CALL :TURNIP_SPRITE %%E[anger] %%B 1
    CALL :TURNIP_SPRITE %%E[surprise] %%C 1
    CALL :TURNIP_SPRITE %%E[shy] %%D 1
    
    CALL :TURNIP_ADD_IMG !%%E! 0 70 2 421 480
    CALL :TURNIP_ADD_SPR %%E[expr] !%%E[anger]! 146 118 3 100 99 99 0
)
...
CALL :TURNIP_IMG bkg "Sprites/bkg.jpg"
CALL :TURNIP_SPRITE dia "Sprites/dia.png" 1

SET /A "dia[y]=550", "dia[fin]=dia[wait]=0", "dia[line]=dia[char]=dia[next]=1", "dia[num]+=1"

CALL :TURNIP_ADD_IMG %bkg% 0 0 0 900 550
CALL :TURNIP_ADD_SPR dia[box] %dia% 0 %dia[y]% 2 100 900 300 0
CALL :TURNIP_ADD_TEXT tex 0 265 2 transparent 900 300 10 "$25$black$left$The Bold Font$25"
CALL :TURNIP_ADD_TEXT stop 0 450 2 transparent 900 100 10 "$25$#ad42f5$center$The Bold Font$25"
        ...
        IF "!dia[next]!" == "0" (
            IF "%%~nK" == "97" (
                IF "!dia[line]!" == "!dia[num]!" (
                    ...
                    SET /A "dia[next]=1", "dia[wait]=frame + 1000"
                    FOR %%E in (!dia[line]!) DO (FOR %%Q in ("!dia[%%E]!") DO (
                        IF "!%%~Q!" == "" (
                            %TURNIP_MSG% %stop%;f$black$25$center$The Bold Font$
                        ) else (
                            SET /A "dia[line]+=1"
                            %TURNIP_MSG% %alina[expr]%;s$!%%~Q!$99$99$100$0$0@ %stop%;f$black$25$center$The Bold Font$
                        )
                    ))
                )
            )
...
IF "!dia[every]!" == "0" (IF "!dia[next]!" == "1" (IF "!dia[fin]!" == "1" (IF !frame! GEQ !dia[wait]! (
    SET /A "dia[char]+=1"
    FOR /F "tokens=1-2" %%A in ("!dia[line]! !dia[char]!") DO (
        IF "!prev!" == "!dia[%%A]:~0,%%B!" (
            SET /A "dia[line]+=1", "dia[char]=0", "dia[next]=0"
            %TURNIP_MSG% %stop%;f$#ad42f5$25$center$The Bold Font$(press A to continue^)
        ) else (
            SET "prev=!dia[%%A]:~0,%%B!"
            %TURNIP_MSG% %tex%;f$black$25$left$The Bold Font$!prev!
        )
    )
))))

SET /A "frame+=1", "every=frame %% 7", "dia[every]=frame %% 100"
%TURNIP_GAME_END%
