...
SET "char[alina]=char.png Alina"
SET story="Hmph%TURNIP_EXCL% Are you really thinking I'd go on a date with you%TURNIP_QUESTION% You're crazy%TURNIP_EXCL% Just who do you think you are%TURNIP_QUESTION%%TURNIP_EXCL%" "I-I mean I'm not trying to be mean, just realistic%TURNIP_EXCL% My parents would never approve." "And it's not l-like you're a bad guy or anything...it's just that I think we're better off as friends." "..." "You know what, fine%TURNIP_EXCL% I-I'll give you a chance, alright%TURNIP_QUESTION% Consider yourself lucky%TURNIP_EXCL%"

FOR /F "tokens=1-2" %%A in ("%char[alina]%") DO (
    CALL :TURNIP_IMG %%B %%A
    CALL :TURNIP_ADD_IMG !%%B! 0 70 2 421 480
)
...
CALL :TURNIP_SPRITE dia "dia.png" 1

SET /A "dia[y]=550", "dia[line]=1", "dia[fin]=0", "dia[char]=dia[next]=1", "dia[num]+=1"

CALL :TURNIP_ADD_IMG %bkg% 0 0 0 900 550
CALL :TURNIP_ADD_SPR dia[box] %dia% 0 %dia[y]% 2 1 900 300 0
...
IF "!dia[every]!" == "0" (IF "!dia[next]!" == "1" (IF "!dia[fin]!" == "1" (
    SET /A "dia[char]+=1"
...
SET /A "frame+=1", "every=frame %% 10", "dia[every]=frame %% 200"
%TURNIP_GAME_END%
