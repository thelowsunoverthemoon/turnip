...
CALL :TURNIP_WINDOW #00000 900 500 100 100 10

SET "ball[num]=30"
FOR /L %%Q in (1, 1, %ball[num]%) DO (
    CALL :TURNIP_ADD_SHAPE ball%%Q 180 300 100 #c997f7 50 50 25
    SET /A "char[%%Q][x]=!RANDOM! %% 900", "char[%%Q][y]=!RANDOM! %% 200 + 300", "dir[%%Q][y]=((!RANDOM! %% 2 * -2) + 1) * (!RANDOM! %% 3 + 1)", "dir[%%Q][x]=((!RANDOM! %% 2 * -2) + 1) * (!RANDOM! %% 3 + 1)", "r[%%Q]=!RANDOM! %% 256", "g[%%Q]=!RANDOM! %% 256", "b[%%Q]=!RANDOM! %% 256"
)
...
IF "!every!" == "0" (
    ...
    IF "!d[var]!" ==  "0" (
        %TURNIP_GET_KEY%
            IF "%%~nK" == "97" (
                FOR /L %%Q in (1, 1, %ball[num]%) DO (
                    SET /A "r[%%Q]=!RANDOM! %% 256", "g[%%Q]=!RANDOM! %% 256", "b[%%Q]=!RANDOM! %% 256"
                )
            )
        %TURNIP_END_KEY%
    )
    FOR /L %%Q in (1, 1, %ball[num]%) DO (
        IF !char[%%Q][y]! LEQ 0 (
            SET "dir[%%Q][y]=!dir[%%Q][y]:-=!"
        ) else IF !char[%%Q][y]! GEQ 450 (
            SET "dir[%%Q][y]=-!dir[%%Q][y]:-=!"
        )
        IF !char[%%Q][x]! LEQ 0 (
            SET "dir[%%Q][x]=!dir[%%Q][x]:-=!"
        ) else IF !char[%%Q][x]! GEQ 850 (
            SET "dir[%%Q][x]=-!dir[%%Q][x]:-=!"
        )

        SET /A "char[%%Q][x]+=dir[%%Q][x]", "char[%%Q][y]+=dir[%%Q][y]"
        SET "msg=!msg!!ball%%Q!;x$!char[%%Q][x]!;y$!char[%%Q][y]!;b$rgb(!r[%%Q]!,!g[%%Q]!,!b[%%Q]!)@ "
    )
    %TURNIP_MSG% !msg:~0,-2!
)
SET /A "frame+=1", "every=frame %% 2", "d[var]=frame %% 13"
%TURNIP_GAME_END%
