...
CALL :TURNIP_WINDOW #dcdedc 915 700 100 100 10

SET "bar[all]=180 220 100 130 50 10 260 400 345 120 298 134 43"
CALL :TURNIP_ADD_TEXT tex 350 100 2 transparent 200 200 0 "Graph Of Data$30$black$center$Segoe UI"
FOR %%G in (%bar[all]%) DO (
    SET /A "b[num]+=1", "b[x]=b[num] * 60 + 10", "b[y]=650 - %%G"

    CALL :TURNIP_ADD_SHAPE b!b[num]! !b[x]! !b[y]! 100 #5ea1b5 50 %%G 0
    CALL :TURNIP_ADD_TEXT tex!b[num]! !b[x]! 670 2 transparent 50 5 0 "%%G$15$black$center$Segoe UI"
    FOR %%N in (!b[num]!) DO (
        CALL :TURNIP_ADD_ATTRIB !b%%N! H
    )
)
...
IF "!every!" == "0" (
    %TURNIP_GET_MOUSE%
        FOR /F "tokens=1-2" %%A in ("%%~nM") DO (
            IF "%%A" == "H" (
                %TURNIP_MSG% %%B;b$#aedbe8
            ) else (
                %TURNIP_MSG% %%B;b$#5ea1b5
            )
        )
    %TURNIP_END_MOUSE%
)
